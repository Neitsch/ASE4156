...
class Shares extends React.Component {
  ...
  valueRow(value: valueShape, i: number, a: Array<valueShape>): React$Element<*> {
    return (
      <TableRow displayBorder={i == a.length - 1} key={i}>
        ...
        <TableRowColumn style={{ textAlign: 'right' }}>{formatMoney(this.props.currency, value.value)}</TableRowColumn>
      </TableRow>
    );
  }
  ...
  render(): React$Element<{}> {
    return this.renderCard(
      this.valueRows(this.props.values),
      this.collectRow(this.props.values),
